...
CustomTransitionEffect::CustomTransitionEffect() :
   m_refCount(1)
{
   m_constants.progress = 0.f;
}

HRESULT __stdcall CustomTransitionEffect::CreateCustomImpl(_Outptr_ IUnknown** ppEffectImpl)
{
   // Since the object's refcount is initialized to 1, we don't need to AddRef here.
   *ppEffectImpl = static_cast<ID2D1EffectImpl*>(new (std::nothrow) CustomTransitionEffect());
...
HRESULT CustomTransitionEffect::Register(_In_ ID2D1Factory1* pFactory)
{
...
IFACEMETHODIMP CustomTransitionEffect::Initialize(
...
)
{
   ...
   PathAppend(szPath, _T("CustomTransitionEffect.cso"));
   ...
   HRESULT hr = pEffectContext->LoadPixelShader(GUID_CustomTransitionPixelShader, pData, dwBytes);
   delete[] pData;
   ...
   if (SUCCEEDED(hr))
   {
      ...
      hr = pTransformGraph->ConnectToEffectInput(2, this, 2);
      hr = pTransformGraph->ConnectToEffectInput(3, this, 3);
...
HRESULT CustomTransitionEffect::SetProgress(float progress)
{
...
float CustomTransitionEffect::GetProgress() const
{
   return m_constants.progress;
}

HRESULT CustomTransitionEffect::UpdateConstants()
{
...
IFACEMETHODIMP CustomTransitionEffect::PrepareForRender(D2D1_CHANGE_TYPE changeType)
{
...
IFACEMETHODIMP CustomTransitionEffect::SetGraph(_In_ ID2D1TransformGraph* pGraph)
{
   return E_NOTIMPL;
}
...
IFACEMETHODIMP CustomTransitionEffect::SetDrawInfo(_In_ ID2D1DrawInfo* pDrawInfo)
{
   HRESULT hr = S_OK;

   m_drawInfo = pDrawInfo;

   hr = m_drawInfo->SetPixelShader(GUID_CustomTransitionPixelShader);
...
IFACEMETHODIMP CustomTransitionEffect::MapOutputRectToInputRects(
...
IFACEMETHODIMP CustomTransitionEffect::MapInputRectsToOutputRect(
...
)
{
   ...
   pOutputRect[2] = pInputRects[2];
   pOutputRect[3] = pInputRects[3];
...
IFACEMETHODIMP CustomTransitionEffect::MapInvalidRect(
   UINT32 inputIndex,
...
IFACEMETHODIMP_(UINT32) CustomTransitionEffect::GetInputCount() const
{
   return 4;
}
...
IFACEMETHODIMP_(ULONG) CustomTransitionEffect::AddRef()
{
   m_refCount++;
   return m_refCount;
}

IFACEMETHODIMP_(ULONG) CustomTransitionEffect::Release()
{
   m_refCount--;
...
IFACEMETHODIMP CustomTransitionEffect::QueryInterface(
   _In_ REFIID riid,
